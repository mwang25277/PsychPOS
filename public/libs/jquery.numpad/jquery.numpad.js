...
(function($){
		...
		return this.each(function(){
			...
			if ($('#'+id).length == 0) {
				...
				table.append(
					...
					$(options.rowTpl)
						.append($(options.cellTpl).append($(options.buttonFunctionTpl).html('').addClass('neg').click()))
	...
	*/
	...
	$.fn.numpad.defaults = {
		target: false,
		openOnEvent: 'click',
		backgroundTpl: '<div class="table-responsive"></div>',
		gridTpl: '<table class="table modal-content" style="width: 430px; height: 500px;"></table>',
		displayTpl: '<input type="text" class="form-control" style="font-size: 16pt; height: 40px;"/>',
		displayCellTpl: '<td colspan="12"></td>',
		rowTpl: '<tr></tr>',
		cellTpl: '<td></td>',
		buttonNumberTpl: '<button type="button" class="btn btn-default" style="width: 75px; height: 75px;"></button>',
		buttonFunctionTpl: '<button type="button" class="btn" style="width: 75px; height: 75px;"></button>',
		gridTableClass: '',
